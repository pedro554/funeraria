    ...
    while True:
        ...
        if event == '-VER_CONTRATO-':
            ...
            try:
                ...
                with open(f"{path}\\temp.png", "wb") as fh:
                    fh.write(base64.decodebytes(img_data))

                contrato_temp = Image.open(f"{path}\\temp.png")
                page_2 = Image.open(f"{path}\\modulos\\contratoPage2.png")
                page_2.show()
                contrato_temp.show()
            except:
                sg.popup('Selecione um contrato')
        ...
        if event == '-ALTERAR-':
            ...
            try:
                contratos = alterarContrato.alterar(banco, cursor, contratos, indice, path)
                janelaContratos['-TABELA_CONTRATOS-'].Update(contratos)
            except:
                sg.popup('Selecione um contrato')
